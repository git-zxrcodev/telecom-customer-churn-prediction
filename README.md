...
│
├── data/
│   ├── 01_raw/                  # Raw Kaggle dataset (7,043 rows)
│   ├── 02_processed/            # Cleaned & type-wrangled data (7,033 rows)
│   ├── 03_featured/             # Engineered feature set (5 new features)
│   └── 04_dashboard/            # Human-readable CSV for BI consumption
│
├── models/                      # Trained model artefacts (.joblib + JSON metadata)
│
├── dashboards/
│   └── customer_churn_dashboard.pbix  # Power BI dashboard
│
├── images/                      # EDA visualisation exports
├── requirements.txt
├── setup.py
└── README.md
```
...
An interactive **Power BI** dashboard (`dashboards/customer_churn_dashboard.pbix`) provides visual representation of churn patterns across the customer base.

### Data Source

The dashboard is powered by `data/04_dashboard/customer_churn_dashboard.csv`, generated by `04_transform_for_dashboard.ipynb`. This file contains the full 7,033-row customer dataset with all engineered features decoded to human-readable labels.
